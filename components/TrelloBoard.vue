...
<template>
  <div>
    ...
    <!-- 5.0 Теперь с помощью спец. пропа "animation" изменим время, за которое анимация будет отображаться на 150 мс, сделав её более плавной. -->
    <!-- 5.1 Что мы ещё можем сделать, так это определить область, нажатие по которой будет включать режим перетаскивания карточки (пока у нас этот режим включается по клику и удержанию по любой поверхности карточки). Делается это при помощи специального пропа "handle", а значением его будет селектор элемента, по нажатию на который будет включаться режим перетаскивания. Назовём этот класс "drag-handle" и будет он у элемента span со специальным эмодзи рядом с заголовком. -->
    <draggable
      v-model="columns"
      group="columns"
      item-key="id"
    ...
    >
      ...
      <!-- 5.2 Вообще-то нам лучше превратить этот элемент в компонент, т.к. он ещё пригодится нам в комп. задачи. -->
      <!-- Go to [components\DragHandle.vue] -->
